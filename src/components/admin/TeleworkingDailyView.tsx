        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
          ...
          <div className="space-y-2">
            {view.office.length === 0 ? (
              ...
              view.office.map(({ usuario, schedule }) => (
                <div
                  key={schedule.id}
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
          ...
          <div className="space-y-2">
            {view.remote.length === 0 ? (
              ...
              view.remote.map(({ usuario, schedule }) => (
                <div
                  key={schedule.id}
                  className="flex items-center justify-between p-3 bg-white/5 rounded-lg hover:bg-white/10 transition-colors"
                >
          ...
          <div className="lg:col-span-2 bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-white/60">
                  {view.unscheduled.length} empleados sin ubicación asignada
                </p>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {view.unscheduled.map((usuario) => (
                <button
                  key={usuario.id}
