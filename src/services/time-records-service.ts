...
export class TimeRecordsService {
  static async createRecord(record: Omit<RegistroTiempo, 'id'>): Promise<RegistroTiempo> {
  ...
  static async createMultipleRecords(records: Omit<RegistroTiempo, 'id'>[]): Promise<RegistroTiempo[]> {
  ...
  static async getLatestRecordByUser(userId: string): Promise<RegistroTiempo | null> {
    const { data, error } = await supabase
      .from('registros_tiempo')
      .select('*')
      .eq('usuario_id', userId)
